...
def create_pptx(images_dict: Dict[int, Image.Image]) -> Presentation:
    ...
    Args:
        images (list): A Dictionary of images to add to the PowerPoint presentation.
...
def create_pptx_with_transcript(images_dict: Dict[int, Image.Image], transcript: Dict[int, str]) -> Presentation:
    """
    Creates a new PowerPoint presentation with the given list of images.
    ...
    for image_index, image in images_dict.items():
        ...
        if image_index in transcript:
            note += f"\nTranscript: {transcript[image_index]}"
