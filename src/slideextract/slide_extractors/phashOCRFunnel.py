...
import json
import logging
from slide_segmentation.slide_extractors.baseSlideExtractor import BaseSlideExtractor
from slide_segmentation.ocr.config import PHASH_THRESHOLD, PHASH_PERCENTILE, DURATION_THRESHOLD, WORD_CT_THRESHOLD, \
    SLIDE_EXTRACT_OUTPUT_BUCKET_NAME, new_slide_clf
from slide_segmentation.slide_extractors.util import *
from slide_segmentation.ocr.ocr_process import textract_resp_to_paragraph
from slide_segmentation.slide_extractors.image_to_ppt import *


logger = logging.getLogger('phashOCRFunnel')


class PhashOCRFunnel(BaseSlideExtractor):

    def __init__(self, *args, **kwargs):
        #super().__init__(*args, **kwargs)
        self.duration_threshold = kwargs.get("duration_threshold", DURATION_THRESHOLD)
        self.word_ct_threshold = kwargs.get("world_ct_threshold",WORD_CT_THRESHOLD)
        self.phash_threshold = kwargs.get("duration_threshold", PHASH_THRESHOLD)
        self.phash_percentile = kwargs.get("world_ct_threshold",PHASH_PERCENTILE)
        # intermediate data for troubleshooting
        self.frames_data = {}
        self.phash_threshold = None
        self.phash_df = pd.DataFrame()
        self.combined_df = pd.DataFrame()
        self.new_slide_images = dict()
        self.ocr_data_dict = dict()
